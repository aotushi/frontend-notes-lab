...
层叠顺序（stacking order）指元素发生重叠时的垂直显示优先级。在同一层叠上下文中，从低到高依次为：

1. 背景和边框（当前层叠上下文元素的背景和边框）
2. 负 `z-index` 的定位元素
3. 普通流中的块级盒（非定位、非浮动）
4. 浮动盒（非定位的浮动元素）
5. 普通流中的行内盒
6. `z-index: 0` 或 `z-index: auto` 的定位元素
7. 正 `z-index` 的定位元素（数值越大越靠上）

注意：`z-index` 只在参与层叠的元素（定位元素、flex item、grid item 等）上生效，且只在同一层叠上下文中比较。如果两个元素处于不同的层叠上下文，子元素的 `z-index` 再大也无法突破父级上下文的整体排序。
